...
export function useVault() {
  ...
  function loadProfileLocal(soulId) {
    ...
    try {
      ...
      // Blob-URLs sind session-gebunden und nach Reload ungültig — überspringen
      if (stored && !stored.startsWith("blob:")) profileUrl.value = stored;
  ...
  // Entkoppelt den Vault-Handle ohne Dateien/Profil zu löschen.
  // Wird beim Soul-Wechsel aufgerufen — der Handle gehört zur alten Soul.
  function disconnectVault() {
    ...
    cloudSource.value  = "";
    // Profil (profileUrl, profileBase64) bleibt erhalten — gehört zur Soul, nicht zum Handle
  }
