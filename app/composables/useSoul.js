...
`;
  ...
  async function refreshCert() {
    ...
    const currentCert = soulContent.value.match(/soul_cert:\s*([a-f0-9]{20,})/i)?.[1]?.trim();
...
async function resetCertToV0() {
  ...
  try {
    ...
    // proof = aktueller Cert aus sys.md (auch wenn Server ihn für Auth ablehnt,
    // ist er als HMAC-Nachweis der Soul-Inhaberschaft gültig)
