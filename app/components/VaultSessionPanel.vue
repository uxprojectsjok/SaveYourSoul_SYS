  ...
  <div :class="headless ? '' : 'rounded-2xl border border-[var(--sys-border)] bg-[var(--sys-bg-surface)] overflow-hidden'">
    ...
    <Transition name="slide-up">
      <div v-if="open || headless" class="px-5 pb-5 space-y-4 border-t border-[var(--sys-border)]">
        <p class="text-xs text-[var(--sys-fg-dim)] pt-4 leading-relaxed">
          Zeitfenster für externe Dienste. Nur wenn der Vault <strong class="text-[var(--sys-fg)]">offen</strong> ist,
        ...
        <div v-if="isUnlocked" class="space-y-3">
          <div
            class="rounded-xl px-4 py-3 flex items-center justify-between border"
            :class="vaultKey
          ...
          >
            <div>
              <p class="text-xs font-medium text-white/75">
                Vault offen{{ vaultKey ? ' · verschlüsselt' : '' }}
              </p>
              <p v-if="!isUnlimited && expiresAt" class="text-xs text-[var(--sys-fg-dim)] mt-0.5">
                Läuft ab: {{ formatTs(expiresAt) }}
              </p>
              <p v-else-if="isUnlimited" class="text-xs text-[var(--sys-fg-dim)] mt-0.5">Kein Ablauf</p>
            </div>
            <button
              class="text-xs px-3 py-1.5 rounded-lg border border-[rgba(239,68,68,0.3)] text-red-400 hover:bg-[rgba(239,68,68,0.1)] transition-colors min-h-[36px]"
              :disabled="loading"
              @click="handleLock"
        ...
        <div v-else class="space-y-3">
          ...
          <div class="grid grid-cols-4 gap-1.5">
            <button
              ...
              :key="opt.value"
              class="py-2 rounded-lg border text-xs font-mono transition-all min-h-[36px]"
          ...
          <div v-if="selectedDuration === 'unlimited'" class="rounded-xl bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.12)] px-3 py-2">
          ...
          <div class="space-y-2">
            ...
            <div class="grid grid-cols-3 gap-1.5">
              <button
                v-for="opt in [{ value: 'none', label: 'Keine' }, { value: 'passkey', label: '🔑 Passkey' }, { value: 'mnemonic', label: '📝 12 Wörter' }]"
                :key="opt.value"
                type="button"
                class="py-2 rounded-lg border text-xs transition-all min-h-[36px]"
            ...
            <Transition name="slide-up">
              <div v-if="methodWarning" class="rounded-xl bg-[rgba(234,179,8,0.08)] border border-[rgba(234,179,8,0.25)] px-3 py-2">
            ...
            <!-- Passkey Info -->
            <Transition name="slide-up">
              <div v-if="encryptMode === 'passkey'" class="rounded-xl bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.12)] px-3 py-2 space-y-1">
            ...
            <Transition name="slide-up">
              <div v-if="encryptMode === 'mnemonic'" class="space-y-2">
                <textarea
                  ...
                  class="w-full bg-[rgba(255,255,255,0.04)] border border-[var(--sys-border)] rounded-xl px-3 py-2 text-xs text-[var(--sys-fg)] placeholder:text-[var(--sys-fg-dim)] resize-none focus:outline-none focus:border-[rgba(255,255,255,0.3)] transition-colors"
                  rows="2"
                  autocomplete="off"
                  autocorrect="off"
                  spellcheck="false"
          ...
          <button
            class="w-full py-3 rounded-xl text-sm font-semibold transition-all min-h-[44px]"
...
const encryptMode      = ref('none')   // 'none' | 'passkey' | 'mnemonic'
...
watch(encryptMode, (mode) => {
  const prev = savedMethod()
  methodWarning.value = ''
  if (!prev || mode === prev || mode === 'none') return
