<template>
  <div :class="syncStatus === 'differs' ? 'overflow-y-auto overscroll-contain h-full' : 'flex flex-col h-full min-h-0'">
    ...
    <!-- ── ÜBER / META (flex-none, immer sichtbar) ─────────────────────── -->
    <div class="px-4 pt-4 pb-3 border-b border-[var(--sys-border)] flex-none">

      <div class="flex items-start justify-between gap-2 mb-1">
        <div class="min-w-0">
          <p class="text-[10px] font-medium tracking-widest uppercase text-[var(--sys-fg-dim)] mb-0.5">Über</p>
          <h2 class="text-sm font-semibold text-[var(--sys-fg)] truncate">
            {{ soulMeta?.name || '#' + (soulMeta?.id?.slice(0, 8) || '------') }}
          </h2>
          <!-- Lokal / Server Status -->
          <div class="flex items-center gap-1.5 mt-1 flex-wrap">
            <span class="inline-flex items-center gap-1 text-[10px] font-mono text-[var(--sys-fg-dim)]">
              <span class="w-1.5 h-1.5 rounded-full flex-none"
                :class="vaultConnected ? 'bg-violet-400' : 'bg-white/30'"></span>
              {{ vaultConnected ? 'Vault · Lokal' : 'Lokal' }}
            </span>
            <span v-if="syncStatus === 'in_sync'"
              class="inline-flex items-center gap-0.5 text-[10px] font-mono text-emerald-400">
              <svg class="w-2.5 h-2.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
                <path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5"/>
              </svg>
              Server ✓
            </span>
            <span v-else-if="syncStatus === 'differs'"
              class="text-[10px] font-mono text-amber-400">Server ≠</span>
            <span v-else-if="syncStatus === 'checking'"
              class="text-[10px] font-mono text-[var(--sys-fg-dim)] animate-pulse">prüfe…</span>
          </div>
        </div>
        <!-- Save flash -->
        <Transition name="fade-quick">
          <div v-if="saveFlash" class="flex items-center gap-1 text-[10px] text-emerald-400 flex-none mt-0.5">
      ...
      <!-- Soul Index (volle Props — Breakdown klappbar) -->
      <div class="mt-3">
        <SoulMaturityMeter
      ...
      <div class="flex flex-wrap gap-1.5 mt-3">
        <span v-if="soulMeta?.created"
          class="text-[10px] font-mono px-2 py-0.5 rounded-full bg-white/[0.05] text-[var(--sys-fg-dim)] border border-[var(--sys-border)]">
          Erstellt {{ soulMeta.created }}
        </span>
        <span v-if="soulMeta?.lastSession"
          class="text-[10px] font-mono px-2 py-0.5 rounded-full bg-white/[0.05] text-[var(--sys-fg-dim)] border border-[var(--sys-border)]">
          Zuletzt {{ soulMeta.lastSession }}
        </span>
      </div>
    </div>
