<template>
  <div class="rounded-xl bg-[var(--sys-bg-elevated)] border border-[var(--sys-border)] overflow-hidden">
    <div class="p-3 space-y-3">

    <!-- ── Tabs ─────────────────────────────────────────────────────────── -->
    <div class="flex gap-1 p-1 rounded-xl bg-white/5 border border-white/8 items-center">
      <button
        v-for="t in TABS" :key="t.id"
        @click="switchTab(t.id)"
        class="flex-1 h-8 rounded-lg text-sm font-medium transition-all"
        :class="tab === t.id ? 'bg-white/12 text-white' : 'text-white/40 hover:text-white/65'"
      >{{ t.label }}</button>
      <button
        @click="onRefresh"
        :disabled="isScanning"
        class="w-8 h-8 flex items-center justify-center rounded-lg text-white/40 hover:text-white hover:bg-white/8 disabled:opacity-30 transition text-lg leading-none"
        title="Aktualisieren"
      >
    ...
    <template v-if="tab === 'local'">
      ...
      <!-- Cloud-Modus-Badge -->
      <div v-if="vaultConnected && memoryMode" class="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/4 border border-white/10 text-xs text-white/50">
      ...
      <template v-else>
        ...
        <div v-if="selectedLocal.size > 0"
          class="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2 px-3 py-2.5 rounded-xl bg-white/5 border border-white/10"
        >
          ...
          <div class="flex items-center gap-1.5">
            <button v-if="soulCert" @click="uploadSelectedLocal"
              class="flex-1 sm:flex-none px-2.5 py-1.5 rounded-lg bg-white/8 text-white/70 hover:text-white hover:bg-white/12 transition text-xs flex items-center justify-center gap-1.5"
            >
            ...
            <button @click="deleteSelectedLocal"
              class="flex-1 sm:flex-none px-2.5 py-1.5 rounded-lg bg-red-950/30 text-red-400/70 hover:text-red-400 hover:bg-red-950/50 transition text-xs flex items-center justify-center gap-1.5"
            >
            ...
            <button @click="clearSelection('local')"
              class="w-7 h-7 flex items-center justify-center rounded-lg text-white/40 hover:text-white hover:bg-white/8 transition text-xs"
            >✕</button>
          </div>
        </div>
        ...
        <div v-for="(files, type) in localArchive" :key="type">
          ...
          <div v-if="playerSrc && playerTab === 'local' && files.includes(playerName)"
            class="px-3 py-2 mb-1 rounded-xl border border-white/8 bg-white/3">
            ...
            <video v-else :src="playerSrc" controls playsinline class="w-full rounded-lg max-h-48"/>
          </div>
          <!-- Datei-Zeilen -->
          <div class="divide-y divide-white/[0.05] rounded-xl border border-white/[0.07]">
            ...
            >
              ...
              <button
                ...
                class="w-8 h-8 flex items-center justify-center rounded-lg transition disabled:opacity-25 shrink-0"
    ...
    <template v-if="tab === 'network'">
      ...
      <template v-else>
        ...
        <label class="flex items-center gap-3 cursor-pointer px-1">
          <div class="relative w-9 h-5 rounded-full transition-colors flex-none"
            :class="publicShare.enabled ? 'bg-[#22c55e]' : 'bg-white/15'">
            <div class="absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform shadow sys-toggle-thumb"
              :class="publicShare.enabled ? 'translate-x-[18px]' : 'translate-x-0.5'"/>
          </div>
          <input type="checkbox" v-model="publicShare.enabled" class="sr-only" @change="onSavePublicEnabled"/>
          <span class="text-xs text-white/75">Netzwerk-Freigaben aktiv</span>
        </label>

        <!-- Datei hinzufügen (direkt unter Toggle) -->
        <button @click="publicPickerOpen = !publicPickerOpen"
          class="w-full flex items-center justify-center gap-1.5 py-2 rounded-xl border border-dashed border-white/15 text-white/40 hover:text-white/70 hover:border-white/30 transition text-xs">
        ...
        <div v-if="publicPickerOpen" class="rounded-xl border border-white/8 bg-white/3 max-h-36 overflow-y-auto">
          <button v-for="f in serverFilesNotPublic" :key="f.name"
            @click="addPublicFile(f.name)"
            class="w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-white/5 transition-colors">
            <span class="text-xs font-mono text-white/60 truncate">{{ f.name }}</span>
            <span class="ml-auto text-xs text-white/35">{{ TYPE_LABELS[f.type] }}</span>
          </button>
        ...
        <div v-for="(files, type) in networkArchive" :key="type">
          ...
          <div class="divide-y divide-white/[0.05] rounded-xl border border-white/[0.07]">
            <div v-for="name in files" :key="name" class="flex items-center gap-2 px-3 min-h-[44px]">
              ...
              <button
                @click.stop="openContextMenu('network', type, name, $event)"
    ...
    <label
      ...
      class="w-full flex items-center justify-center gap-1.5 py-2 rounded-xl border border-dashed border-white/15 text-white/40 hover:text-white/70 hover:border-white/30 transition cursor-pointer text-xs"
    ...
    >
      ...
      <input
        ...
        type="file"
        multiple
        accept="audio/*,video/*,image/*,.md,.txt"
        class="sr-only"
    ...
    <div v-if="soulCert" class="pt-3 space-y-2">
      ...
      <button
        class="w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-150 text-left"
  ...
  <Teleport to="body">
    <Transition name="vault-toast">
      <div
        ...
        class="fixed bottom-6 left-1/2 -translate-x-1/2 z-[300] px-4 py-3 rounded-xl border shadow-2xl text-sm font-medium flex items-center gap-2.5 max-w-sm w-[calc(100vw-2rem)]"
        :class="successMsg
      ...
      >
        <div class="relative w-full max-w-sm bg-[var(--sys-bg-elevated)] border border-[var(--sys-border)] rounded-2xl shadow-2xl overflow-hidden">
          <div class="flex items-center justify-between px-5 pt-5 pb-3">
            ...
            <button @click="confirmDeleteOpen = false" class="w-7 h-7 flex items-center justify-center rounded-lg text-white/40 hover:text-white hover:bg-white/8 transition" aria-label="Schließen">✕</button>
          </div>
          <div class="px-5 pb-5 space-y-4">
            ...
            <div class="flex gap-2">
              <button
                ...
                class="flex-1 h-10 rounded-xl border border-[var(--sys-border)] text-sm text-white/50 hover:text-white/80 hover:bg-white/5 transition"
              >Abbrechen</button>
              <button
                @click="handleDeleteVault"
                :disabled="deleteVaultLoading"
                class="flex-1 h-10 rounded-xl bg-red-500/15 border border-red-500/30 text-sm font-semibold text-red-400 hover:bg-red-500/25 disabled:opacity-40 transition"
      ...
      >
        <div class="relative w-full max-w-sm bg-[var(--sys-bg-elevated)] border border-[var(--sys-border)] rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div class="flex items-center justify-between px-5 pt-5 pb-3">
            ...
            <button @click="syncModalOpen = false" class="w-7 h-7 flex items-center justify-center rounded-lg text-white/40 hover:text-white hover:bg-white/8 transition" aria-label="Schließen">✕</button>
          </div>
          ...
          <div class="px-5 pb-2 space-y-3">
            ...
            <div class="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/[0.04] border border-white/8 text-xs">
          ...
          <div class="flex items-center gap-2 px-5 py-4 border-t border-[var(--sys-border)]">
            <button
              ...
              class="flex-1 py-2.5 rounded-xl border border-white/12 text-white/55 text-xs hover:bg-white/6 hover:text-white transition"
            >Abbrechen</button>
            <button
              @click="confirmSync"
              class="flex-1 py-2.5 rounded-xl bg-[var(--sys-violet)] text-white text-xs font-medium hover:opacity-90 active:scale-[0.97] transition"
            >Jetzt syncen</button>
          </div>
  ...
  <Teleport to="body">
    <div v-if="openMenuKey"
      class="fixed z-[200] w-48 rounded-xl border border-white/12 bg-[var(--sys-bg-elevated)] shadow-2xl py-1"
    ...
    >
      ...
      <template v-else-if="menuCtx.tab === 'network'">
        <button
          @click="onUploadNetworkFile(menuCtx.name); closeMenu()"
          class="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-left text-white/70 hover:bg-white/8 hover:text-white transition"
        >
          <svg class="w-4 h-4 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
            <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5m-13.5-9L12 3m0 0 4.5 4.5M12 3v13.5"/>
        ...
        <div class="my-1 border-t border-white/8"/>
        <button
          @click="onDeleteNetworkFile(menuCtx.name); closeMenu()"
          class="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-left text-red-400/80 hover:bg-red-950/30 hover:text-red-400 transition"
        >
          <svg class="w-4 h-4 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
            <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"/>
...
const serverFilesNotPublic = computed(() => {
  ...
  return allServerFiles.value.filter(f => !already.has(f.name));
});
...
async function uploadToServer(type, name) {
  ...
  try {
    ...
    const res = await syncFile(props.soulCert, serverType, name, file, key);
    if (res.ok) {
      await fetchVpsVaultFiles(props.soulCert);
...
async function downloadToLocal(type, name) {
  ...
  try {
    ...
    else showError("Lokal speichern fehlgeschlagen");
...
function addPublicFile(name) {
  publicShare.value.public_files.push({ name, cipher: "open" }); // immer offen
...
async function onSavePublicEnabled() {
  if (!props.soulCert) return;
...
async function handleDeleteVault() {
  ...
  try {
    ...
    await loadContext(props.soulCert);
    showSuccess("Cloud-Vault gelöscht ✓");
  } catch (e) {
...
// localFiles beim Mount und bei Verbindungsänderung neu einlesen
watch(vaultConnected, async (connected) => {
  if (connected) await scanApiVault();
}, { immediate: true });
</script>
