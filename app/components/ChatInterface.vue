  ...
  <div class="sys-chat">
    ...
    <div ref="scrollEl" class="stream">
      ...
      <div v-if="!messages.length" class="empty">
        <div class="kicker">Session · leere Seite</div>
        <p class="prompt">
          Beginne, indem du <em>aussprichst</em>, woran du heute denkst.<br>
          <span class="mode-hint">{{ localRole === 'soul' ? 'Deine Soul antwortet dir.' : 'Deine Soul liest mit.' }}</span>
        </p>
        <div class="hint">↵ Enter zum Senden · ⇧↵ neue Zeile</div>
      </div>
...
.empty::after {
  ...
  top: -20%; right: -10%; width: 240px; height: 240px;
  background: radial-gradient(circle, rgba(139,92,246,0.22) 0%, transparent 60%);
  pointer-events: none; filter: blur(30px);
}
.kicker {
  font-family: var(--mono); font-size: 10px; text-transform: uppercase;
  letter-spacing: 0.24em; color: var(--fg-3);
  margin-bottom: 18px; padding-bottom: 10px;
  border-bottom: 2px solid var(--accent); display: inline-block;
}
.prompt {
  font-family: var(--serif); font-size: clamp(22px,3vw,28px); line-height: 1.35;
  color: var(--fg); margin: 0 0 16px; font-weight: 400; letter-spacing: -0.01em;
  position: relative; z-index: 1; text-wrap: balance;
}
.prompt em  { font-style: italic; color: var(--accent-bright); }
.mode-hint  { font-style: italic; color: var(--fg-3); font-size: 0.85em; }
.hint {
  font-family: var(--mono); font-size: 11px; letter-spacing: 0.18em;
  text-transform: uppercase; color: var(--fg-3); position: relative; z-index: 1;
}
...
@media (max-width: 700px) {
  .dock-main { grid-template-columns: auto 1fr auto; }
  .mode-btn  { min-width: 64px; padding: 0 10px; letter-spacing: 0.12em; }
  .send      { width: 52px; }
  .chip      { padding: 0 10px; font-size: 9px; }
}
@media (max-width: 480px) {
  .stream { padding: 20px 16px 48px; }
