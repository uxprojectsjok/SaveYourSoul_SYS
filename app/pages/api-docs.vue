...
Authorization: Bearer &lt;dein-service-token&gt;</DocCode>
          ...
          <!-- ═══════════════════════════════════════
               SECTION 6e – AGENT MARKETPLACE
               ═══════════════════════════════════════ -->
          <section :id="'marketplace'" class="doc-section mb-16 scroll-mt-20">
            <DocHeading level="1" badge="Monetarisierung">Agent Marketplace</DocHeading>
            <p class="doc-lead">Jede Soul kann zu einem bezahlten KI-Dienst werden. Externe KI-Agenten entdecken deine Soul im dezentralen IPFS-Verzeichnis, zahlen POL auf Polygon Mainnet und erhalten zeitlich begrenzten MCP-Zugang — vollautomatisch, ohne zentrale Plattform.</p>

            <DocHeading level="2">Wie es funktioniert — 4 Schritte</DocHeading>
            <div class="doc-steps mb-6">
              <div class="doc-step">
                <div class="doc-step-num">1</div>
                <div>
                  <p class="text-sm font-semibold mb-0.5">Entdecken — <code class="doc-code text-xs">soul_discover</code></p>
                  <p class="text-sm text-[var(--sys-fg-dim)]">Ein externer KI-Agent ruft <code class="doc-code">soul_discover</code> auf. Das Tool durchsucht das IPFS-Verzeichnis nach registrierten SYS-Souls. Der Agent bekommt MCP-Endpunkt, Wallet-Adresse, pol_per_request und free_tools zurück. Vor der Wallet-Adresse erscheint immer ein Risikohinweis.</p>
                </div>
              </div>
              <div class="doc-step">
                <div class="doc-step-num">2</div>
                <div>
                  <p class="text-sm font-semibold mb-0.5">Risiko bestätigen — <code class="doc-code text-xs">soul_pay(user_confirmed: false)</code></p>
                  <p class="text-sm text-[var(--sys-fg-dim)]">Das MCP-Tool <code class="doc-code">soul_pay</code> gibt beim ersten Aufruf (user_confirmed: false) einen vollständigen Risikohinweis zurück. Die KI muss dem Nutzer diesen Text zeigen und explizite Zustimmung einholen.</p>
                </div>
              </div>
              <div class="doc-step">
                <div class="doc-step-num">3</div>
                <div>
                  <p class="text-sm font-semibold mb-0.5">Zahlen — native POL-Transaktion</p>
                  <p class="text-sm text-[var(--sys-fg-dim)]">Der Nutzer sendet POL (≥ pol_per_request) an die in der Soul konfigurierte Wallet. Anschließend wird <code class="doc-code">soul_pay(tx_hash, user_confirmed: true)</code> aufgerufen. Der Server prüft die TX on-chain und gibt einen <code class="doc-code">pol_access_token</code> zurück.</p>
                </div>
              </div>
              <div class="doc-step">
                <div class="doc-step-num">4</div>
                <div>
                  <p class="text-sm font-semibold mb-0.5">Zugriff nutzen — Bearer am MCP-Endpunkt</p>
                  <p class="text-sm text-[var(--sys-fg-dim)]">Der pol_access_token (48 Hex) wird als Bearer-Token am MCP-Endpunkt verwendet. Er gilt für alle <code class="doc-code">free_tools</code> für die vom Soul-Besitzer festgelegte Dauer (1h · 12h · 1d · 7d · 30d · 90d max).</p>
                </div>
              </div>
            </div>

            <DocHeading level="2">Einrichten — Voraussetzungen</DocHeading>
            <div class="doc-info-box my-4">
              <div class="space-y-1.5 text-xs text-[var(--sys-fg-dim)]">
                <div class="flex gap-2"><span class="text-white/70 flex-none">1.</span> <strong class="text-[var(--sys-fg)]">Polygon-Anker</strong> — Soul muss on-chain verifiziert sein (SoulRegistry Contract). Ohne Anker kann amortization nicht aktiviert werden.</div>
                <div class="flex gap-2"><span class="text-white/70 flex-none">2.</span> <strong class="text-[var(--sys-fg)]">Wallet konfigurieren</strong> — Polygon Mainnet Wallet-Adresse unter <em>PUT /api/soul/amortization</em> eintragen (pol_per_request, wallet, free_tools, token_duration).</div>
                <div class="flex gap-2"><span class="text-white/70 flex-none">3.</span> <strong class="text-[var(--sys-fg)]">IPFS-Pinning-JWT</strong> (optional) — Für IPFS-Registrierung: JWT eines IPFS-Pinning-Dienstes über die App-UI eintragen (kein SSH nötig). Gespeichert in <code class="doc-code">/var/lib/sys/pinata_jwt</code>.</div>
                <div class="flex gap-2"><span class="text-white/70 flex-none">4.</span> <strong class="text-[var(--sys-fg)]">IPFS registrieren</strong> — <em>POST /api/soul/register</em> pinnt ERC-8004-Metadaten auf IPFS. Soul ist danach über <code class="doc-code">soul_discover</code> auffindbar.</div>
              </div>
            </div>

            <DocHeading level="2">Einnahmen</DocHeading>
            <p class="doc-p">Alle Zahlungen werden in <code class="doc-code">/var/lib/sys/souls/{soul_id}/earnings.json</code> protokolliert. Die App fragt alle 30 Sekunden <code class="doc-code">GET /api/soul/earnings</code> ab und zeigt eine Toast-Benachrichtigung bei neuen Zahlungen. Über das MCP-Tool <code class="doc-code">soul_earnings</code> kann die KI Einnahmen direkt abfragen.</p>

            <div class="doc-warning-box my-4">
              <p class="text-xs font-semibold text-[var(--sys-amber)] mb-1">Blockchain-Transaktionen sind endgültig</p>
              <p class="text-xs text-[var(--sys-fg-dim)] leading-relaxed">Zu wenig gezahlt = kein Zugang, POL unwiederbringlich verloren. Zu viel gezahlt = Überschuss geht an den Soul-Besitzer. Falsche Wallet-Adresse = kein Rückgabe-Mechanismus. Deshalb zeigt <code class="doc-code">soul_discover</code> immer zuerst den Risikohinweis, und <code class="doc-code">soul_pay</code> erfordert <code class="doc-code">user_confirmed: true</code>.</p>
            </div>

            <p class="doc-p">Alle Details zur Einrichtung findest du im Abschnitt <strong>Häufige Fragen</strong> am Ende dieser Seite — „Was ist der Amortisierungs-Stack?", „Wie funktioniert die POL-Zahlung technisch?", „Wie richte ich die IPFS-Registrierung ein?"</p>
          </section>
          ...
          <section :id="'security'" class="doc-section mb-16 scroll-mt-20">
            ...
            <p class="doc-p">Der Vault-Zugang ist zeitlich begrenzt. Verfügbare Dauern: <code class="doc-code">1h</code> <code class="doc-code">12h</code> <code class="doc-code">1d</code> <code class="doc-code">30d</code> <code class="doc-code">182d</code> <code class="doc-code">365d</code> <code class="doc-code">unlimited</code>. Beim Öffnen ist zwingend ein Verschlüsselungsschlüssel erforderlich — entweder <strong>Passkey</strong> (biometrisch, gerätgebunden) oder <strong>12 Wörter</strong> (BIP39, portabel). Unverschlüsselter Zugang ist nicht möglich.</p>
          ...
          <section :id="'mcp-protocol'" class="doc-section mb-16 scroll-mt-20">
            ...
            <div class="doc-info-box my-4">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1 text-xs text-[var(--sys-fg-dim)] font-mono">
                <span>soul_read</span><span class="text-white/35">sys.md laden — Soul-Besitzer: vollständige sys.md; zahlender Agent (pol_access_token): nur AGENT-Block</span>
                <span>soul_write</span><span class="text-white/35">sys.md aktualisieren — Soul-Besitzer: beliebige ## Section; zahlender Agent: nur AGENT-Block (section-Parameter ignoriert)</span>
                <span>soul_maturity</span><span class="text-white/35">Reife-Score berechnen und in sys.md schreiben</span>
                <span>soul_skills</span><span class="text-white/35">Skill-Dateien aus Soul + .md/.txt-Kontext-Dateien generieren</span>
                <span>soul_discover</span><span class="text-white/35">IPFS-Marketplace durchsuchen — findet registrierte Souls nach Name/ID, gibt MCP-Endpunkt + Zahlungskonditionen zurück. Risikohinweis wird vor Wallet-Adresse angezeigt.</span>
                <span>soul_earnings</span><span class="text-white/35">POL-Einnahmen abrufen — total_pol, total_requests, letzte 20 Einträge (tx_hash, wallet, amount, timestamp)</span>
                <span>soul_pay</span><span class="text-white/35">POL-Transaktion einlösen → pol_access_token. Erfordert user_confirmed: true (kein Default) — ohne Bestätigung gibt das Tool nur den Risikohinweis zurück, keine Blockchain-Aktion.</span>
                <span>vault_manifest</span><span class="text-white/35">Übersicht aller Vault-Inhalte — synced_files, active_files, Berechtigungen und verfügbare Endpunkte</span>
                ...
                <span>audio_list / audio_get</span><span class="text-white/35">Sprachaufnahmen abrufen</span>
                <span>image_list / image_get</span><span class="text-white/35">Bild als base64 direkt an KI liefern — Gesichtsanalyse ohne Upload</span>
                <span>video_list / video_get</span><span class="text-white/35">Video-Frames extrahieren und als Bilder ausgeben — Bewegungsanalyse direkt im Chat</span>
                <span>context_list / context_get</span><span class="text-white/35">Kontext-Dokumente abrufen (.md und .txt — PDF/DOCX werden übersprungen)</span>
                <span>calendar_read</span><span class="text-white/35">## Calendar-Abschnitt aus sys.md abrufen</span>
                ...
                <span>verify_human</span><span class="text-white/35">Identitätsverifikation — prüft biometrische oder Dokument-basierte Identität</span>
                <span>soul_cloud_push</span><span class="text-white/35">Verschlüsseltes .soul-Bundle an externen Speicher pushen (Arweave, IPFS, S3) — API-Key nur im RAM</span>
                <span>elevenlabs_agent_update</span><span class="text-white/35">ElevenLabs-Agent mit aktueller Soul + allen Profilen aktualisieren — API-Key wird nur für diesen Call im RAM gehalten, nie gespeichert</span>
              </div>
            </div>
            ...
            <DocHeading level="2">AGENT-Block: Sandbox für externe Agenten</DocHeading>
            <p class="doc-p">Zahlende externe Agenten (pol_access_token) erhalten keinen Zugriff auf die vollständige sys.md — nur auf den <strong>AGENT-Block</strong>: einen abgegrenzten Bereich innerhalb der sys.md, der ausschließlich für externe Agenten reserviert ist.</p>
            <DocCode lang="text">&lt;!-- AGENT:START --&gt;
Deine öffentlich sichtbare Agent-Beschreibung, Expertise, Angebote …
&lt;!-- AGENT:END --&gt;</DocCode>
            <p class="doc-p">Wenn kein AGENT-Block existiert, ist der Block leer — keine privaten Daten werden geleakt. Externe Agenten können nur in diesen Block schreiben (<code class="font-mono" style="color:var(--sys-violet)">soul_write</code>), nie in andere Abschnitte. Die Marker werden beim Schreiben automatisch entfernt und neu gesetzt (Injection-Schutz).</p>
...
await elevenlabs_agent_update({
  agent_id:           "deine-agent-id",
  elevenlabs_api_key: "sk_...",   // nur für diesen Call im RAM, nie gespeichert
  language:           "de"
})
// → liest sys.md + alle Profile → baut System-Prompt → patcht Agent via ElevenLabs API</DocCode>
          </section>
...
const nav = [
  ...
  {
    id: 'marketplace-nav', title: 'Agent Marketplace',
    items: [
      { id: 'marketplace', title: 'Übersicht & Setup' },
    ]
  },
  {
...
const quickLinks = [
  ...
  { id: 'marketplace',       title: 'Agent Marketplace',    desc: 'Soul monetarisieren — Pay-per-Access mit Polygon POL' },
...
const coreConcepts = [
  ...
  { term: 'Marketplace',  desc: 'Opt-in Monetarisierungsschicht. Soul-Betreiber aktivieren pay-per-access (POL) — externe KI-Agenten entdecken die Soul via IPFS, zahlen Polygon-Token und erhalten einen zeitlich begrenzten Zugangs-Token.' },
]
...
const endpoints = [
  ...
  },
  {
    ...
    desc: 'POL-Zahlung für MCP-Zugang. Body: { soul_id, tx_hash }. Validiert TX auf Polygon Mainnet: Empfänger muss amortization.wallet entsprechen, Betrag ≥ pol_per_request, mindestens 1 Bestätigung. Replay-geschützt (48h shared dict + pol_used_tx.json). Bei Erfolg: access_token als Bearer-Token für MCP-Anfragen. Laufzeit = amortization.token_duration (max. 90 Tage). Wird normalerweise nicht direkt aufgerufen — Agenten nutzen das MCP-Tool soul_pay(), das vorher einen Risikohinweis erzwingt (user_confirmed: true).',
    response: '{ "ok": true, "access_token": "48hex...", "expires_at": "2026-07-25T14:00:00Z", "pol_amount": "0.003", "soul_id": "abc123", "from": "0x..." }'
  },
  {
  ...
  },
  {
    ...
    desc: 'Pinnt ERC-8004-kompatible Agent-Metadaten auf IPFS. Erfordert Polygon-Verifikation (amortization.verified_wallet) und konfigurierten IPFS-Pinning-JWT (via UI oder .env). IPFS-keyvalues-Index (schema: saveyoursoul/soul/1.0) macht die Soul im Agent-Marketplace auffindbar. Speichert agent_registry_cid + agent_registry_url + agent_registered_at in api_context.json.',
    response: '{\n  "ok": true,\n  "cid": "bafk...",\n  "ipfs_uri": "ipfs://bafk...",\n  "gateway_url": "https://ipfs.io/ipfs/bafk...",\n  "pinned_at": "2026-04-21T..."\n}'
  },
  {
    method: 'GET/PUT/DELETE', path: '/api/soul/pinata-config', permission: 'soul_cert',
    desc: 'IPFS-Pinning-JWT über die UI konfigurieren — kein Server-SSH nötig. GET gibt { configured, preview } zurück (JWT gemaskiert: erste 8 + letzte 4 Zeichen). PUT speichert den JWT in /var/lib/sys/pinata_jwt. DELETE löscht ihn. soul-mcp liest die Datei bei jeder Anfrage — kein Neustart des MCP-Servers erforderlich. .env-Variable PINATA_JWT hat Vorrang falls gesetzt.',
    response: '{ "configured": true, "preview": "eyJhbGci…4abc" }'
  },
  {
    method: 'GET', path: '/api/soul/register-preview', permission: 'soul_cert',
    desc: 'Vorschau des ERC-8004-kompatiblen JSON-Blobs der gepinnt würde — kein Schreibvorgang, kein IPFS-Aufruf.',
    response: '{ "soul_id": "...", "name": "...", "mcp_endpoint": "...", "amortization": {...} }'
  },
  {
    method: 'GET', path: '/api/soul/paid-read', permission: 'pol_access_token',
    desc: 'AGENT-Block aus sys.md für zahlende externe Agenten. Bearer = pol_access_token (48 Hex, TTL 1h–90d). Nur wenn amortization.enabled=true. Gibt ausschließlich den Inhalt zwischen <!-- AGENT:START --> und <!-- AGENT:END --> zurück — kein Zugriff auf die vollständige sys.md. Verschlüsselte souls werden mit 403 abgelehnt.',
    response: '{ "soul_content": "<!-- AGENT:START -->\\n...AGENT-Block...\\n<!-- AGENT:END -->" }'
  },
  {
    method: 'POST', path: '/api/soul/paid-beme', permission: 'pol_access_token',
    desc: 'beme_chat für zahlende externe Agenten — liest sys.md serverseitig, ruft Anthropic auf, antwortet als Soul-Besitzer. Nur wenn amortization.enabled=true.',
    response: '{ "response": "..." }'
  },
  {
    method: 'GET', path: '/api/soul/paid-context', permission: 'pol_access_token',
    desc: 'Kontext-Dokumente für zahlende externe Agenten — Liste oder spezifisches Dokument.',
    response: '{ "files": [...] }'
  },
  {
    method: 'GET', path: '/api/soul/paid-profile/{type}', permission: 'pol_access_token',
    desc: 'Analyse-Profil (face/voice/motion/expertise) für zahlende externe Agenten.',
    response: '{ "type": "face", "data": {...} }'
  },
  {
...
const refImplRequirements = [
  ...
  { id: 'R-12', text: 'Agent Marketplace: POST /api/soul/pay muss TX on-chain validieren (Empfänger, Betrag ≥ pol_per_request, ≥1 Bestätigung) bevor pol_access_token (48 Hex) ausgestellt wird. Dual-layer Replay-Schutz: shared dict + persistente Datei.' },
  { id: 'R-13', text: 'pol_access_token: im shared dict pol_access + /var/lib/sys/pol_tokens/{token}.json persistieren. TTL nach amortization.token_duration (Whitelist: 1h/12h/1d/7d/30d/90d). GET /internal/validate-pol-token muss localhost-only bleiben.' },
]
...
const faqItems = ref([
  ...
  { q: 'Was ist der Amortisierungs-Stack?', a: 'Ein optionaler Monetarisierungs-Layer für Soul-Betreiber. Jede Soul hat einen Zugangsmodus — Grundeinstellung ist immer "Frei" (alle MCP-Tools ohne Zahlung nutzbar). Der Modus kann im Agent-Marketplace-Panel per Toggle auf "Bezahlt (POL)" umgestellt werden: Die Soul verlangt dann pro MCP-Anfrage eine POL-Zahlung (Polygon Native Token). Externe KI-Agenten entdecken die Soul im IPFS-Marketplace (soul_discover), senden POL an die konfigurierte Wallet, posten den tx_hash an /api/soul/pay und erhalten einen pol_access_token — mit einer Laufzeit von 1h bis 90 Tagen (festgelegt durch den Soul-Besitzer in amortization.token_duration). Der Token wird als Bearer am MCP-Endpunkt verwendet. Bestimmte Tools können als free_tools markiert werden und bleiben auch im Bezahlt-Modus kostenfrei zugänglich. Einnahmen werden in earnings.json protokolliert und sind über GET /api/soul/earnings abrufbar. Die App pollt alle 30s und zeigt bei neuen Zahlungen eine Toast-Benachrichtigung.', open: false },
  { q: 'Wie funktioniert die POL-Zahlung technisch?', a: 'Der Agent sendet eine native POL-Transaktion auf Polygon Mainnet an die in amortization.wallet konfigurierte Adresse (Betrag ≥ pol_per_request). Das MCP-Tool soul_pay() erzwingt vorher einen Risikohinweis und verlangt user_confirmed: true — erst dann wird die TX eingereicht. Der Server prüft: (1) Amortisierung ist aktiv, (2) tx_hash-Format korrekt, (3) TX existiert on-chain, (4) Empfänger stimmt exakt überein, (5) Betrag ≥ pol_per_request, (6) mindestens 1 Bestätigung, (7) TX wurde nicht bereits verwendet (dualer Replay-Schutz: 48h shared dict pol_tx_used + persistente pol_used_tx.json). Bei Erfolg: pol_access_token (48 Hex-Zeichen, TTL = amortization.token_duration des Soul-Besitzers, max. 90 Tage) + pol_amount + from_wallet. Fehler: 402 (Amortisierung deaktiviert), 404 (TX nicht gefunden), 422 (falscher Empfänger / Betrag zu niedrig / unbestätigt), 409 (TX bereits verwendet).', open: false },
  { q: 'Was ist der IPFS Agent-Marketplace und wie funktioniert die Entdeckung?', a: 'Bei POST /api/soul/register baut der Server einen ERC-8004-kompatiblen JSON-Blob (soul_id, Name, MCP-Endpunkt, Zahlungskonditionen, Wallet, Verifikations-URL) und pinnt ihn via konfiguriertem IPFS-Pinning-Dienst. Die keyvalues-Metadaten (schema: saveyoursoul/soul/1.0, soul_id) machen alle registrierten Souls durchsuchbar — ohne vorherige Kenntnis der MCP-URL. Ein externer Agent ruft das MCP-Tool soul_discover auf (oder GET /internal/discover-souls direkt) und bekommt: mcp_endpoint, pay_endpoint, wallet, pol_per_request. Das IPFS-Verzeichnis ist unabhängig vom SoulRegistry-Contract — beide Systeme koexistieren.\n\nWelchen IPFS-Pinning-Dienst du verwendest, entscheidest du selbst. Der JWT wird über die App-UI eingetragen (kein SSH nötig).', open: false },
  { q: 'Wie richte ich die IPFS-Registrierung Schritt für Schritt ein?', a: 'Voraussetzung: Die Soul muss Polygon-verifiziert sein (Blockchain-Anker gesetzt, GET /api/soul/verify gibt "verified": true zurück). Ohne Verifikation wird die Registrierung mit 403 blockiert.\n\nSchritt 1 — IPFS-Pinning-JWT beschaffen: Einen IPFS-Pinning-Dienst deiner Wahl nutzen → Account anlegen → API-Key / JWT erzeugen → kopieren.\n\nSchritt 2 — JWT über die UI eintragen (kein SSH nötig): Im SYS-Dashboard → "Agent Marketplace" → "IPFS-Schlüssel" → JWT einfügen → "Speichern". Der JWT wird sofort in /var/lib/sys/pinata_jwt gespeichert und ist ohne MCP-Server-Neustart aktiv.\n\nAlternativ via .env: PINATA_JWT=eyJhbG... in soul-mcp/.env eintragen + pm2 restart soul-mcp. .env hat Vorrang vor der UI-Konfiguration.\n\nSchritt 3 — Zugangsmodus wählen: Im Panel → "Zugangsmodus" → Toggle "Frei / Bezahlt (POL)". Frei ist die Grundeinstellung und wird sofort gespeichert. Für Bezahlt-Modus: pol_per_request + Wallet + kostenlose Tools eintragen → speichern (erfordert Polygon-Verifikation). Alternativ: PUT /api/soul/amortization.\n\nSchritt 4 — Soul registrieren: Im Panel → "IPFS-Registrierung" → "Auf IPFS registrieren". Gibt cid, ipfs_uri, gateway_url zurück. CID wird in api_context.json gespeichert.\n\nSchritt 5 — Verifikation: gateway_url im Browser öffnen — der JSON-Blob muss sichtbar sein. Mit soul_discover (q=dein_name) im MCP-Client prüfen ob die Soul im Marketplace erscheint.\n\nDomain-Wechsel: Erneut "Auf IPFS registrieren" — neue CID mit aktualisierten Endpunkten.', open: false },
  { q: 'Welche Daten stehen im gepinnten IPFS-Blob?', a: 'Der ERC-8004-kompatible JSON-Blob enthält: soul_id (UUID), name (Plaintext-Name aus sys.md), mcp_endpoint (vollständige URL), soul_endpoint (/api/soul), pay_endpoint (/api/soul/pay), verify_endpoint (/api/soul/verify), amortization-Objekt (enabled, pol_per_request, wallet, free_tools[]), agent_type ("soul"), protocol ("saveyoursoul/1.0"), schema_version ("ERC-8004/draft"). Die keyvalues-Metadaten enthalten zusätzlich: soul_id, schema ("saveyoursoul/soul/1.0"), registered (Zeitstempel) — diese werden für den IPFS-keyvalues-Filter genutzt. Wichtig: Name und MCP-Endpunkt sind dauerhaft öffentlich auf IPFS — nur registrieren wenn das gewünscht ist.', open: false },
