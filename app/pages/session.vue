  ...
  <ClientOnly>
    ...
    <div class="sys-session" v-if="!certValidating">
      ...
      <footer class="sess-foot">
        <span class="sf-copy">© {{ new Date().getFullYear() }} SaveYourSoul</span>
        <NuxtLink to="/datenschutz" class="sf-link">Datenschutz</NuxtLink>
        <NuxtLink to="/impressum" class="sf-link">Impressum</NuxtLink>
        <NuxtLink to="/lizenz" class="sf-link">Lizenz</NuxtLink>
      </footer>
...
@media (max-width: 900px) { .sess-head { grid-template-columns: auto auto; grid-template-rows: auto auto; gap: 12px; } .sess-head .pill { grid-column: 1/-1; grid-row: 2; justify-self: start; border-left: 0; padding-left: 0; font-size: 10px; } .sess-head .tools { overflow-x: auto; max-width: 100%; scrollbar-width: none; } .sess-head .tools::-webkit-scrollbar { display: none; } }
...
.tool { padding: 10px 16px; border-left: 1px solid var(--rule); border-top: 0; border-bottom: 0; border-right: 0; font-family: var(--mono); font-size: 10px; letter-spacing: 0.2em; text-transform: uppercase; color: var(--fg-3); cursor: pointer; background: transparent; }
...
@media (max-width: 900px) { .sess-body { grid-template-columns: 1fr; } .mobile-hidden { display: none; } }
.mobile-hidden { /* overridden in desktop MQ */ }
@media (min-width: 901px) { .mobile-hidden { display: flex; } }
...
@media (max-width: 900px) { .col-soul { border-right: 0; border-bottom: 1px solid var(--rule); } }
...
/* Legal footer */
.sess-foot { display: flex; align-items: center; gap: 0; padding: 0 clamp(16px,3vw,32px); border-top: 1px solid var(--rule); background: var(--paper-3); font-family: var(--mono); font-size: 9px; letter-spacing: 0.18em; text-transform: uppercase; color: var(--fg-4); min-height: 36px; }
.sf-copy { color: var(--fg-4); margin-right: 20px; }
.sf-link { color: var(--fg-3); text-decoration: none; padding: 0 14px; border-left: 1px solid var(--rule); min-height: 36px; display: flex; align-items: center; }
