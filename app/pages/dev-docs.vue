...
pol_access_token (48 hex chars)
    └── Issued by /api/soul/pay after on-chain POL TX verification
            └── Scoped to soul_id + free_tools[]
                    └── Time-limited (duration set by soul owner)</DocCode>
...
token     = HMAC-SHA256(vault_key, soul_id)[hex]</DocCode>
          ...
          <DocHeading level="2">pol_access_token — Paid Agent Authentication</DocHeading>
          <p class="doc-p">48 lowercase hex characters. Issued by <code class="doc-code">POST /api/soul/pay</code> after the server verifies a Polygon on-chain payment. No dots — this is how vault_auth.lua distinguishes it from soul_cert.</p>
          <DocCode>Detection (Lua): #token == 48 and token:match("^[0-9a-fA-F]+$") and not token:find(".", 1, true)</DocCode>
          <p class="doc-p">The token is stored in <code class="doc-code">ngx.shared.pol_access</code> (fast, in-memory) and on disk at <code class="doc-code">/var/lib/sys/pol_tokens/{token}.json</code> (survives restarts). It grants access only to tools listed in the soul's <code class="doc-code">free_tools</code> configuration — nothing else. Requests outside the allowed prefixes return <code class="doc-code">403 paid_agent_restricted</code>.</p>

          <DocHeading level="2">Auth Guard Precedence (vault_auth.lua)</DocHeading>
          <div class="doc-steps">
            <div class="doc-step">
              <div class="doc-step-num">1</div>
              <div><p class="text-sm font-semibold mb-0.5">soul_cert check</p><p class="text-sm text-[var(--sys-fg-dim)]">Bearer {uuid}.{32hex} — full owner access</p></div>
            </div>
            <div class="doc-step">
              <div class="doc-step-num">2</div>
              <div><p class="text-sm font-semibold mb-0.5">pol_access_token check</p><p class="text-sm text-[var(--sys-fg-dim)]">Bearer {48hex} — paid agent, restricted to free_tools</p></div>
            </div>
            <div class="doc-step">
              <div class="doc-step-num">3</div>
              <div><p class="text-sm font-semibold mb-0.5">service_token check</p><p class="text-sm text-[var(--sys-fg-dim)]">Bearer {64hex} or X-Webhook-Token or ?token= — scoped service access</p></div>
            </div>
          </div>
          <p class="doc-p">First match wins. All failures return 401 (or 403 vault_locked for service-tokens when vault is locked).</p>
...
Discovery:        https://YOUR_DOMAIN/.well-known/oauth-authorization-server</DocCode>
          ...
          <DocHeading level="2">Access Modes</DocHeading>
          <div class="doc-table-wrapper">
            <table class="doc-table">
              <thead><tr><th>Mode</th><th>Auth</th><th>soul_read</th><th>soul_write</th></tr></thead>
              <tbody>
                <tr>
                  <td class="font-semibold">Soul Owner</td>
                  <td class="text-xs text-[var(--sys-fg-dim)]">OAuth 2.0 + PKCE</td>
                  <td class="text-xs text-[var(--sys-fg-dim)]">Full sys.md</td>
                  <td class="text-xs text-[var(--sys-fg-dim)]">Any named ## Section</td>
                </tr>
                <tr>
                  <td class="font-semibold">Paid Agent</td>
                  <td class="text-xs text-[var(--sys-fg-dim)]">pol_access_token (48 hex)</td>
                  <td class="text-xs text-[var(--sys-fg-dim)]">AGENT block only</td>
                  <td class="text-xs text-[var(--sys-fg-dim)]">AGENT block only · markers stripped</td>
                </tr>
              </tbody>
            </table>
          </div>

          <DocHeading level="2">AGENT Block (paid agent sandbox)</DocHeading>
          <p class="doc-p">sys.md may contain a sandboxed section for external agent writes:</p>
          <DocCode>&lt;!-- AGENT:START --&gt;
Agent-written content here.
&lt;!-- AGENT:END --&gt;</DocCode>
          <p class="doc-p">Only one AGENT block per file. If no block exists, <code class="doc-code">soul_write</code> (paid) appends one. Content containing the markers is stripped before writing — agents cannot escape the block regardless of input.</p>
          ...
          <div class="doc-table-wrapper">
            <table class="doc-table">
              ...
              <tbody>
                <tr><td><code class="doc-code">replace</code></td><td class="text-[var(--sys-fg-dim)]">Replace entire section / block content</td></tr>
                <tr><td><code class="doc-code">append</code></td><td class="text-[var(--sys-fg-dim)]">Add content after existing content</td></tr>
                <tr><td><code class="doc-code">prepend</code></td><td class="text-[var(--sys-fg-dim)]">Add content before existing content</td></tr>
              </tbody>
            </table>
          </div>
          <p class="doc-p mt-2">If the target section does not exist it is created. For paid agents, <code class="doc-code">section</code> is ignored — write always targets the AGENT block.</p>
        </section>
...
# Decryption happens client-side</DocCode>
          ...
          <DocHeading level="2">Paid Agent Flow (Agent Marketplace)</DocHeading>
          <DocCode lang="bash">BASE="https://YOUR_DOMAIN"
SOUL_ID="uuid-of-target-soul"

# 1. Discover registered souls
curl -s "$BASE/api/soul/discover?q=AI+expert" | jq '.results[0]'
# → { soul_id, name, mcp_endpoint, price_pol, wallet, free_tools, access_duration_hours }

# 2. Pay on-chain (POL on Polygon) → receive access token
ACCESS_TOKEN=$(curl -s -X POST "$BASE/api/soul/pay" \
  -H "Content-Type: application/json" \
  -d "{\"tx_hash\":\"0x...\",\"soul_id\":\"$SOUL_ID\"}" \
  | jq -r '.access_token')

# 3. Read AGENT block (only the sandboxed section of sys.md)
curl -s "$BASE/api/soul/paid-read" \
  -H "Authorization: Bearer $ACCESS_TOKEN"

# 4. Write to AGENT block
curl -s -X POST "$BASE/api/soul/paid-write" \
  -H "Authorization: Bearer $ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"content":"Analysis complete.","mode":"replace"}'
# → { "ok": true, "section": "Agent-Kontext", "mode": "replace" }</DocCode>
  ...
  "mcpServers": {
    "saveyoursoul": {
      ...
      "env": { "MCP_SERVER_URL": "https://YOUR_DOMAIN/mcp" }
        ...
        <!-- ═══ AGENT MARKETPLACE ═══ -->
        <section id="marketplace-overview" class="doc-section mb-16 scroll-mt-20">
          <DocHeading level="1" badge="Agent Marketplace">Overview</DocHeading>
          <p class="doc-lead">The Agent Marketplace turns a SYS soul into a monetizable identity asset. External AI agents discover registered souls via a decentralized IPFS directory, pay in POL (Polygon token) per session, and receive a time-limited access token for MCP tool access.</p>

          <p class="doc-p">This is not a hosted marketplace. The discovery layer is fully decentralized (IPFS with ERC-8004-compatible keyvalues — any compatible pinning service). The payment layer is on-chain (Polygon Mainnet). The access layer is the existing MCP server. SaveYourSoul provides the glue — verification, replay protection, token issuance, and earnings tracking.</p>

          <DocHeading level="2">Architecture</DocHeading>
          <div class="doc-info-box my-6 font-mono text-xs text-[var(--sys-fg-dim)] leading-relaxed">
            <div>soul_discover()  →  IPFS keyvalues index            →  soul metadata</div>
            <div>soul_pay()       →  POST /api/soul/pay              →  Polygon TX verification</div>
            <div>                    ethers.js (soul-mcp, port 3098)  →  access_token</div>
            <div>soul_read_paid() →  GET /api/soul/paid-read         →  sys.md (plaintext only)</div>
            <div>soul_earnings()  →  GET /api/soul/earnings          →  POL received, request count</div>
          </div>
          ...
          <div class="doc-table-wrapper">
            <table class="doc-table">
              ...
              <tbody>
                <tr><td>Polygon anchor</td><td>Soul must be on-chain verified (SoulRegistry contract). Enables amortization.</td></tr>
                <tr><td>IPFS Pinning JWT</td><td>Required for IPFS registration. Set via UI (<code class="doc-code">PUT /api/soul/pinata-config</code>) or <code class="doc-code">PINATA_JWT</code> env var. Any compatible IPFS pinning service works.</td></tr>
                <tr><td>Wallet address</td><td>Polygon Mainnet. Set in amortization config. Receives POL directly.</td></tr>
                <tr><td>cipher_mode: open</td><td><code class="doc-code">paid-read</code> rejects encrypted sys.md — soul must be readable without vault key.</td></tr>
                <tr><td>amortization.enabled</td><td>Set via <code class="doc-code">PUT /api/soul/amortization</code>. Requires verified wallet.</td></tr>
              </tbody>
            </table>
          </div>
        </section>

        <section id="marketplace-flow" class="doc-section mb-16 scroll-mt-20">
          <DocHeading level="1" badge="Agent Marketplace">Payment Flow</DocHeading>
          <p class="doc-lead">The complete flow from discovery to access. All steps execute via MCP tools — no custom integration needed for agents.</p>

          <DocHeading level="2">Step 1 — Discovery</DocHeading>
          <DocCode lang="js">// Agent calls soul_discover with a topic query
soul_discover({ q: "Rechtsanwalt", amortized: true })

// Returns for each registered soul:
// name, soul_id, mcp_endpoint, pol_per_request, wallet, token_duration, tools_after_payment</DocCode>

          <DocHeading level="2">Step 2 — Risk Acknowledgment (mandatory)</DocHeading>
          <p class="doc-p">Before any payment is processed, <code class="doc-code">soul_pay</code> must be called with <code class="doc-code">user_confirmed: false</code> first. The tool returns a plain-text risk warning — the agent must display it to the user and obtain explicit confirmation.</p>
          <DocCode lang="js">// First call — returns risk warning only, no blockchain interaction
soul_pay({
  soul_id: "...",
  tx_hash: "0x...",
  user_confirmed: false   // ← mandatory first step
})

// Returns:
// ⚠️ ZAHLUNGSRISIKO — BITTE VOR DEM SENDEN LESEN:
// 1. ZU WENIG GEZAHLT: TX durch, kein Zugang, POL verloren.
// 2. ZU VIEL GEZAHLT: Überschuss geht an Soul-Besitzer. Keine Rückerstattung.
// 3. FALSCHE WALLET: Zahlung an falsche Adresse nicht rückgängig zu machen.
// 4. BLOCKCHAIN-TRANSAKTIONEN SIND ENDGÜLTIG.</DocCode>

          <div class="doc-warning-box my-4">
            <p class="text-xs font-semibold text-[var(--sys-amber)] mb-1">Design rationale</p>
            <p class="text-xs text-[var(--sys-fg-dim)]">The <code class="doc-code">user_confirmed</code> parameter is a schema-level enforcement mechanism. An AI that passes <code class="doc-code">true</code> without showing the warning violates its own safety guidelines. Claude will not do this. The parameter cannot be defaulted or skipped — it is a required boolean with no default value.</p>
          </div>

          <DocHeading level="2">Step 3 — Send POL</DocHeading>
          <p class="doc-p">The user (or agent operator) sends POL to the wallet address shown in <code class="doc-code">soul_discover</code> output. The amount must be ≥ <code class="doc-code">pol_per_request</code>. Overpayment is accepted; underpayment fails verification and the POL is lost.</p>

          <DocHeading level="2">Step 4 — Redeem TX</DocHeading>
          <DocCode lang="js">// Second call — with user_confirmed: true after user acknowledged the risks
soul_pay({
  soul_id: "...",
  tx_hash: "0xabc...",    // Polygon TX hash from the wallet
  user_confirmed: true    // ← only after user explicitly confirmed
})

// Returns on success:
// access_token: 48-hex-char token
// expires_at:   ISO timestamp (max 90 days from now)
// pol_amount:   actual amount verified on-chain</DocCode>

          <DocHeading level="2">Step 5 — Use Access Token</DocHeading>
          <DocCode lang="js">// Paid tools accept access_token as bearer
soul_read_paid({ access_token: "abc123..." })   // → sys.md plaintext
context_list({ access_token: "abc123..." })     // → context file list
image_list({ access_token: "abc123..." })       // → image list</DocCode>
          <p class="doc-p">The access token is validated by the MCP server against <code class="doc-code">pol_access</code> shared dict and <code class="doc-code">/var/lib/sys/pol_tokens/</code> on disk. Both stores are checked — the disk store survives OpenResty restarts.</p>

          <DocHeading level="2">Token Duration</DocHeading>
          <div class="doc-table-wrapper">
            <table class="doc-table">
              <thead><tr><th>Value</th><th>Duration</th><th>Note</th></tr></thead>
              <tbody>
                <tr><td><code class="doc-code">1h</code></td><td>1 hour</td><td>Minimum — for testing</td></tr>
                <tr><td><code class="doc-code">12h</code></td><td>12 hours</td><td></td></tr>
                <tr><td><code class="doc-code">1d</code></td><td>1 day</td><td></td></tr>
                <tr><td><code class="doc-code">7d</code></td><td>7 days</td><td></td></tr>
                <tr><td><code class="doc-code">30d</code></td><td>30 days</td><td>Recommended default</td></tr>
                <tr><td><code class="doc-code">90d</code></td><td>90 days</td><td>Maximum. Enforced server-side.</td></tr>
              </tbody>
            </table>
          </div>
          <p class="doc-p">Values outside this whitelist fall back to <code class="doc-code">1h</code>. The soul owner sets the duration — the paying agent has no influence on it.</p>
        </section>

        <section id="marketplace-security" class="doc-section mb-16 scroll-mt-20">
          <DocHeading level="1" badge="Agent Marketplace">Security Model</DocHeading>

          <DocHeading level="2">Replay Protection</DocHeading>
          <p class="doc-p">Each TX hash can only be redeemed once. Protection is two-layer:</p>
          <ul class="doc-ul">
            <li><strong>Shared dict</strong> <code class="doc-code">pol_tx_used</code> — in-memory, 48h TTL per entry. Fast O(1) lookup.</li>
            <li><strong>Disk</strong> <code class="doc-code">/var/lib/sys/souls/{id}/pol_used_tx.json</code> — persistent, survives restarts. Up to 10,000 entries per soul.</li>
          </ul>

          <DocHeading level="2">TX Verification</DocHeading>
          <p class="doc-p">The MCP server verifies on Polygon Mainnet before issuing any token:</p>
          <ul class="doc-ul">
            <li>TX must exist and be confirmed (receipt.status === 1, ≥1 block confirmation)</li>
            <li>Recipient (<code class="doc-code">tx.to</code>) must match <code class="doc-code">amortization.wallet</code> (case-insensitive)</li>
...

{
  "soul_id": "...",
  "cert_version": 1,
  "proof": "current_valid_cert"   ← required for existing souls
}</DocCode>

          <DocHeading level="2">Access Token Storage</DocHeading>
          <p class="doc-p">Tokens are stored in two places:</p>
          <ul class="doc-ul">
            <li><code class="doc-code">ngx.shared.pol_access</code> — OpenResty shared memory. Fast validation per request. TTL = token duration.</li>
            <li><code class="doc-code">/var/lib/sys/pol_tokens/{token}.json</code> — disk fallback. The MCP server reads this directly. Cleaned up automatically when TTL expires.</li>
          </ul>
        </section>

        <section id="marketplace-earnings" class="doc-section mb-16 scroll-mt-20">
          <DocHeading level="1" badge="Agent Marketplace">Earnings</DocHeading>
          <p class="doc-lead">Every successful payment is recorded in <code class="doc-code">/var/lib/sys/souls/{id}/earnings.json</code>. The soul owner can query it at any time.</p>

          <DocHeading level="2">GET /api/soul/earnings</DocHeading>
          <DocCode lang="http">GET /api/soul/earnings
Authorization: Bearer {soul_id}.{cert}</DocCode>
          <DocCode lang="json">{
  "ok": true,
  "total_pol": "0.045000",
  ...
  "entries": [
    {
      ...
      "from": "0x...",
      "pol_amount": "0.003",
      "confirmed_at": "2026-04-26T14:22:11Z",
      "redeemed_at":  "2026-04-26T14:22:15Z"
    }
  ]
}</DocCode>
          <p class="doc-p">Up to 10,000 entries per soul. Oldest entries are trimmed first. Only accessible with a valid soul_cert — never via service-token or pol_access_token.</p>

          <DocHeading level="2">In-App Notification</DocHeading>
          <p class="doc-p">The SYS frontend polls <code class="doc-code">GET /api/soul/earnings</code> every 30 seconds while a soul is loaded. When <code class="doc-code">total_requests</code> increases, a toast notification appears bottom-right: <em>"+0.003 POL erhalten — Zahlung von 0xabc…1234"</em>. Polling stops on soul switch and tab close.</p>
          <p class="doc-p">For out-of-app notifications: any Polygon wallet app (MetaMask, Rainbow, Polygonscan watchlist) will show incoming transactions independently — no webhook integration needed.</p>

          <DocHeading level="2">MCP Tool: soul_earnings</DocHeading>
          <DocCode lang="js">soul_earnings()
// Returns: total_pol, total_requests, last 20 entries with from/amount/timestamp
// Auth: soul_cert (owner only). Not accessible via pol_access_token.</DocCode>
        </section>
            ...
            <table class="doc-table">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="text-[var(--sys-fg-dim)]">access</td>
                  <td class="text-xs text-[var(--sys-fg-dim)]">soul_cert · pol_access_token · service_token · ?token=</td>
                  <td class="text-xs text-[var(--sys-fg-dim)]">soul_id, vault_key, via_webhook, service_permissions, is_paid_agent</td>
                </tr>
              </tbody>
            </table>
          </div>
...
PINATA_JWT=                     # IPFS Pinning JWT — .env has priority; fallback is /var/lib/sys/pinata_jwt (set via UI, any compatible service)
...
const nav = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'marketplace-flow',     title: 'Payment Flow' },
      { id: 'marketplace-security', title: 'Security Model' },
      { id: 'marketplace-earnings', title: 'Earnings' },
    ]
  },
  {
    id: 'architecture',
...
const principles = [
  ...
  { p: 'Opt-in monetization', impl: 'Amortization disabled by default — soul owner enables POL pay-per-access; blockchain anchor required' },
]
...
const concepts = [
  ...
  { term: 'soul_cert', def: '32 hex chars. HMAC-SHA256(SOUL_MASTER_KEY, soul_id)[0:32]. Stateless auth token for the soul owner.' },
  { term: 'pol_access_token', def: '48 hex chars. Issued by /api/soul/pay after on-chain POL TX verification. Grants access to free_tools[] for paying external agents. TTL 1h–90d, set by soul owner in amortization.token_duration.' },
...
const standards = [
  ...
  { std: 'ERC-8004', rel: 'Agent identity standard — NFT per agent, three Polygon registries (Identity/Reputation/Validation). SYS is the complementary user-identity layer: ERC-8004 agents discover and pay human souls via the Agent Marketplace. A SYS soul can simultaneously register as an ERC-8004 participant.' },
  { std: 'Polygon (POL)', rel: 'Native token for on-chain soul verification + pay-per-access payments' },
]
...
const trustModel = [
  { id: 'Soul owner', key: 'soul_cert (32 hex)', level: 'Full', scope: 'All operations on own soul' },
  { id: 'Paid agent', key: 'pol_access_token (48 hex)', level: 'Payment-gated', scope: 'Tools listed in free_tools[] — issued after on-chain POL TX verification. TTL set by soul owner.' },
...
const mcpTools = [
  { name: 'soul_read', perm: 'soul · pol_access_token', desc: 'Soul owner: full sys.md. Paid agent: only the AGENT block (<!-- AGENT:START --> … <!-- AGENT:END -->).' },
  { name: 'soul_write', perm: 'soul · pol_access_token', desc: 'Soul owner: targets a named ## Section. Paid agent: writes to AGENT block only. Markers stripped before write (injection protection). mode: replace | append | prepend.' },
  { name: 'soul_maturity', perm: 'soul · pol_access_token', desc: 'Compute the maturity score (0–100) from sys.md completeness.' },
  { name: 'soul_skills', perm: 'soul · pol_access_token', desc: 'Generate Claude skill files from sys.md sections + context files (.md/.txt only). One skill per section, one per context doc, one root skill with biometric profiles.' },
  ...
  { name: 'context_list', perm: 'context_files · pol_access_token', desc: 'List synced context files (.md and .txt only — binary files in the context directory are skipped).' },
  { name: 'context_get', perm: 'context_files · pol_access_token', desc: 'Get a specific context file. Only .md and .txt are accessible — PDF/DOCX in the context directory are not listed or retrievable.' },
  ...
  { name: 'soul_earnings',  perm: 'soul (owner only)', desc: 'Read POL earnings — total_pol received, total_requests count, last 20 payment entries (tx_hash, wallet, amount, timestamp). Not accessible via pol_access_token.' },
  { name: 'soul_discover', perm: 'none (public)', desc: 'Search IPFS agent marketplace for registered SYS souls — no auth required. Returns mcp_endpoint, price_pol, wallet, free_tools, access_duration_hours, maturity.' },
  { name: 'soul_pay',      perm: 'none (public)', desc: 'Redeem a POL transaction for MCP access. Requires user_confirmed: true (mandatory boolean, no default). Without it the tool returns RISK_WARNING text only — no blockchain call. When confirmed: verifies TX on-chain → issues pol_access_token valid for duration set by soul owner.' },
  { name: 'calendar_read', perm: 'calendar · pol_access_token', desc: 'Read the ## Calendar section of sys.md.' },
  { name: 'beme_chat',     perm: 'soul', desc: 'Talk as soul — AI responds as the soul owner. Server reads sys.md, builds system prompt, calls Anthropic API server-side. No client API key needed.' },
]
...
const endpoints = [
  ...
  { method: 'POST', path: '/api/vault/unlock', auth: 'soul_cert', desc: 'Unlock vault with AES key. duration options: 1h · 12h · 1d · 30d · 182d · 365d · unlimited (expires_at=0).' },
  ...
  { method: 'GET/PUT', path: '/api/soul/amortization', auth: 'soul_cert', desc: 'Read or update amortization config (enabled, pol_per_request, wallet, free_tools[], token_duration, cipher_mode). token_duration whitelist: 1h · 12h · 1d · 7d · 30d · 90d (max). PUT enabled=true requires prior Polygon verification via /api/soul/verify.' },
  { method: 'POST',    path: '/api/soul/pay',          auth: 'none',      desc: 'POL payment for MCP access. Body: {soul_id, tx_hash}. Validates TX on-chain (recipient, amount ≥ pol_per_request, confirmations). Dual-layer replay protection: shared dict pol_tx_used (48h TTL) + disk pol_used_tx.json. Issues pol_access_token with TTL set by soul owner (1h · 12h · 1d · 7d · 30d · 90d max). MCP tool soul_pay wraps this endpoint and requires user_confirmed: true.' },
  { method: 'GET',           path: '/api/soul/earnings',      auth: 'soul_cert', desc: 'POL earnings ledger — total_pol, total_requests, entries[] per payment.' },
  { method: 'POST',          path: '/api/soul/register',      auth: 'soul_cert', desc: 'Pin ERC-8004 agent metadata to IPFS via configured pinning service. Requires Polygon verification + IPFS Pinning JWT. Stores CID in api_context.json.' },
  { method: 'GET/PUT/DELETE', path: '/api/soul/pinata-config', auth: 'soul_cert', desc: 'Read or set IPFS Pinning JWT (env var: PINATA_JWT). Stored in /var/lib/sys/pinata_jwt — read at runtime, no soul-mcp restart needed. DELETE clears the key.' },
  // Zugangsmodus + pol_access_token Flow
  { method: 'GET',  path: '/api/soul/paid-read',             auth: 'pol_access_token', desc: 'Returns the AGENT block from sys.md for paying external agents. Rejects encrypted souls with 403. Only active when amortization.enabled=true.' },
  { method: 'POST', path: '/api/soul/paid-write',            auth: 'pol_access_token', desc: 'Write to the AGENT block (<!-- AGENT:START --> … <!-- AGENT:END -->) in sys.md. Requires soul_write in free_tools. Content containing markers is stripped (injection protection). mode: replace | append | prepend.' },
  { method: 'POST', path: '/api/soul/paid-beme',             auth: 'pol_access_token', desc: 'beme_chat for paying external agents — reads sys.md server-side, calls Anthropic, returns AI response as soul owner. Only when amortization.enabled=true.' },
  { method: 'GET',  path: '/api/soul/paid-context',          auth: 'pol_access_token', desc: 'Context files for paying external agents. Returns list or specific context document.' },
  { method: 'GET',  path: '/api/soul/paid-earnings',         auth: 'pol_access_token', desc: 'Earnings summary for paying agents. Returns total_pol, total_requests.' },
...
const rateLimits = [
  { zone: 'chat', rate: '2 r/s', burst: '20', eps: '/api/chat, /api/soul-update, /api/beme, vision' },
  ...
  { zone: 'pay', rate: '3 r/s', burst: '5', eps: '/api/soul/pay' },
]
...
const luaScripts = [
  ...
  { file: 'soul_register.lua',       phase: 'content', purpose: 'POST /api/soul/register — pins ERC-8004 agent metadata to IPFS (calls MCP /internal/pin-json)' },
  { file: 'soul_pinata_config.lua',  phase: 'content', purpose: 'GET/PUT/DELETE /api/soul/pinata-config — stores IPFS Pinning JWT in /var/lib/sys/pinata_jwt; soul-mcp reads it at runtime without restart' },
  ...
  { file: 'soul_paid_beme.lua',    phase: 'content', purpose: 'POST /api/soul/paid-beme — beme_chat for paying external agents. pol_access_token auth, reads sys.md + calls Anthropic server-side.' },
  { file: 'soul_paid_context.lua', phase: 'content', purpose: 'GET /api/soul/paid-context — context files for paying external agents. pol_access_token auth.' },
  { file: 'soul_paid_earnings.lua', phase: 'content', purpose: 'GET /api/soul/paid-earnings — earnings read for paying agents. pol_access_token auth.' },
  { file: 'soul_paid_profile.lua', phase: 'content', purpose: 'GET /api/soul/paid-profile — profile read for paying external agents. pol_access_token auth.' },
  ...
  { file: 'soul_register_preview.lua', phase: 'content', purpose: 'GET /api/soul/register-preview — preview the ERC-8004 JSON blob before pinning. No write, no auth side-effect.' },
