    ...
    <template v-if="hasSoul">
      <div class="sys-page">
        ...
        <div class="sys-dash-body">
          ...
          <section class="col-right">
            ...
            <article v-for="n in journal" :key="n.id" class="note">
              <div class="when">{{ n.when[0] }}<br>{{ n.when[1] }}</div>
              <div>
                <h4>{{ n.title }}</h4>
                <p>{{ n.body }}</p>
              </div>
        ...
        <footer class="dash-foot">
          <span>© 2026 · UX-Projects YOUR_NAME · <NuxtLink to="/datenschutz">Datenschutz</NuxtLink> · <NuxtLink to="/impressum">Impressum</NuxtLink> · <NuxtLink to="/lizenz">Lizenz</NuxtLink></span>
        </footer>
      </div>
    </template>
    ...
    <template v-else>
      <div class="sys-page landing">
        ...
        <nav class="l-nav">
          ...
          <div class="center"><span class="notice-text">Geschlossene Anwendung von UX-Projects YOUR_NAME. Kein öffentlicher Betrieb. Testphase unter Realbedingungen für eingeladene oder interne Nutzerinnen oder Nutzer.</span></div>
          <div class="actions">
        ...
        <section class="sec">
          ...
          <ul class="timeline">
            ...
            <li class="active"><span class="phase">Beta · Live</span><span class="date">2026</span><h4>Erweiterte Identität</h4><div class="chips"><span>Soul Network</span><span>Mehrere Soul-Versionen</span><span>Erweiterte API-Docs</span><span>Offenes Protokoll</span></div></li>
    ...
    <Teleport to="body">
      <Transition name="sys-modal">
        <div v-if="setupOpen" class="sys-modal-wrap" role="dialog" aria-modal="true" @click.self="setupOpen = false">
          <div class="absolute inset-0 bg-black/70 backdrop-blur-sm" @click="setupOpen = false"></div>
          <div class="sys-modal-panel">
            <div class="sys-modal-head">
              ...
              <button class="sys-modal-close" @click="setupOpen = false" aria-label="Schließen">✕</button>
            </div>
        ...
        <div v-if="filesOpen" class="sys-modal-wrap" role="dialog" aria-modal="true" @click.self="filesOpen = false">
          ...
          <div class="sys-modal-panel sys-modal-panel--wide">
            <div class="sys-modal-head">
              ...
              <button class="sys-modal-close" @click="filesOpen = false" aria-label="Schließen">✕</button>
            </div>
            <div class="sys-modal-body">
              <VaultExplorer
                :soul-cert="soulToken"
...
const journal = computed(() => {
  ...
  return entries.slice(-4).reverse().map((e, i) => {
    let when = [e.dateStr, '']
    try {
    ...
    const dot = e.body.indexOf('. ')
    const title = dot > 0 && dot < 80 ? e.body.slice(0, dot) : e.body.slice(0, 72)
    const body  = dot > 0 && dot < 80 ? e.body.slice(dot + 2) : ''
    return { id: i, when, title, body, tag: 'Log' }
  })
})
</script>
...
.dash-foot { padding: 16px clamp(20px,4vw,44px); border-top: 1px solid var(--rule); font-family: var(--mono); font-size: 10px; letter-spacing: 0.18em; text-transform: uppercase; color: var(--fg-4); }
.dash-foot a { color: var(--fg-4); text-decoration: none; }
.dash-foot a:hover { color: var(--accent); }
...
.note { padding: 22px 0; border-bottom: 1px solid var(--rule); display: grid; grid-template-columns: 90px 1fr auto; gap: 24px; align-items: baseline; }
.note .when { font-family: var(--mono); font-size: 11px; letter-spacing: 0.14em; color: var(--fg-3); white-space: nowrap; }
.note h4 { font-family: var(--serif); font-weight: 400; font-size: 22px; margin: 0 0 6px; letter-spacing: -0.015em; color: var(--fg); }
.note p { font-size: 13px; line-height: 1.6; color: var(--fg-2); margin: 0; max-width: 56ch; }
.note .tag { font-family: var(--mono); font-size: 10px; letter-spacing: 0.18em; text-transform: uppercase; color: var(--accent-bright); padding: 4px 8px; border: 1px solid rgba(139,92,246,0.35); white-space: nowrap; align-self: start; }
@media (max-width: 640px) { .note { grid-template-columns: 1fr; gap: 6px; } .note .tag { justify-self: start; } }
...
.login-sheet {
  ...
  padding: 20px 28px 40px; max-height: 92dvh; overflow-y: auto;
...
.login-alt { display: flex; align-items: center; justify-content: space-between; gap: 12px; width: 100%; padding: 18px 20px; background: transparent; border: 1px solid var(--rule); color: var(--fg); cursor: pointer; text-align: left; font: inherit; transition: all 0.15s; }
.login-alt:hover { border-color: var(--accent); background: var(--accent-2); }
.login-alt span { font-family: var(--serif); font-size: 18px; letter-spacing: -0.01em; }
.login-alt-sub { font-family: var(--mono); font-size: 10px; letter-spacing: 0.14em; text-transform: uppercase; color: var(--fg-3); font-family: var(--mono); flex: 1; }
...
.sys-modal-head {
  ...
  padding: 24px 28px 20px; border-bottom: 1px solid var(--rule);
}
...
.sys-modal-title { font-family: var(--serif); font-weight: 400; font-size: 28px; letter-spacing: -0.02em; margin: 0; color: var(--fg); line-height: 1; }
.sys-modal-title em { font-style: italic; color: var(--accent); }
.sys-modal-close { width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; border: 1px solid var(--rule); background: transparent; color: var(--fg-3); cursor: pointer; font-size: 12px; flex-shrink: 0; margin-top: 4px; }
...
.sys-modal-enter-active, .sys-modal-leave-active { transition: opacity 0.2s ease, transform 0.25s cubic-bezier(0.32,0.72,0,1); }
.sys-modal-enter-from, .sys-modal-leave-to { opacity: 0; transform: scale(0.96) translateY(8px); }
</style>
