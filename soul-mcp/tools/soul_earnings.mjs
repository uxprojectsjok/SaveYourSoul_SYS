...
export function register(server, token) {
  const isPaid = token && token.length === 48 && /^[0-9a-f]+$/.test(token);
  ...
  server.tool(
    ...
    isPaid ? { access_token: z.string().optional().describe('pol_access_token aus soul_pay()') } : {},
    async ({ access_token } = {}) => {
      try {
        const useToken = access_token || (isPaid ? token : null);
        const endpoint = (useToken && useToken.length === 48)
          ? '/api/soul/paid-earnings'
          : '/api/soul/earnings';
        const data = await getJson(endpoint, useToken || token);
