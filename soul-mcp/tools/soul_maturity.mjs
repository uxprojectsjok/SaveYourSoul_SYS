...
export function register(server, token) {
  server.tool(
    ...
    [
      ...
      '',
      'Optional: access_token aus soul_pay() für Zugriff auf fremde Souls.',
      'Hinweis: Mit access_token werden Archiv- und Netzwerk-Scores übersprungen (kein Vault-Zugang).',
    ].join('\n'),
    {
      access_token: z.string().optional().describe('pol_access_token aus soul_pay() — für Zugriff auf fremde Souls'),
    },
    async ({ access_token } = {}) => {
      const useToken = access_token || token;
      // isPaid: entweder explizit übergeben ODER Token ist ein pol_access_token (48 hex)
      const isPaid   = !!access_token || /^[0-9a-f]{48}$/i.test(token);
      try {
        const md = await getText(isPaid ? '/api/soul/paid-read' : '/api/soul', useToken);
        ...
        // Archiv via /api/context (nur für eigene Soul verfügbar)
        let archiv = 0;
        if (!isPaid) {
          try {
            const ctx = await getJson('/api/context', useToken);
        ...
        // Netzwerk (nur für eigene Soul verfügbar)
        let netzwerk = 0;
        if (!isPaid) {
          try {
            const net = await getJson('/api/vault/connections', useToken);
