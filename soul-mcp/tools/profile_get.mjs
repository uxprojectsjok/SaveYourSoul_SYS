...
export function register(server, token) {
  server.tool(
    ...
    [
      ...
      '',
      'Optional: access_token aus soul_pay() für Profile fremder Souls (nur unverschlüsselte Profile).',
    ].join('\n'),
    {
      type:         z.enum(['face', 'voice', 'motion', 'expertise']).describe('Profiltyp'),
      access_token: z.string().optional().describe('pol_access_token aus soul_pay() — für Profile fremder Souls'),
    },
    async ({ type, access_token }) => {
      const useToken = access_token || token;
      const path     = `/api/vault/profile/${type}`;
      try {
