...
export function register(server, token) {
  server.tool(
    ...
    {},
    async () => {
      const isPaid = /^[0-9a-f]{48}$/i.test(token);
      try {
        const [soulMd, contextFiles, profiles] = await Promise.all([
          getText(isPaid ? '/api/soul/paid-read' : '/api/soul', token),
...
async function loadContextFiles(token) {
  try {
    ...
    for (const f of files) {
      // Nur Textdateien (.md, .txt) — PDF/DOCX etc. überspringen (zu groß, nicht lesbar)
      if (!f.name || !/\.(md|txt)$/i.test(f.name)) continue;
      try {
...
async function loadPaidContextFiles(token) {
  try {
    const data = await getJson('/api/soul/paid-context', token);
    const files = data?.files ?? [];
    return files.map(f => ({ name: f.name, content: f.content ?? '' })).filter(f => f.content);
  } catch {
    return [];
  }
}
...
async function loadPaidProfiles(token) {
  const profiles = {};
  for (const type of ['face', 'voice', 'motion', 'expertise']) {
    try {
      const data = await getJson(`/api/soul/paid-profile/${type}`, token);
      profiles[type] = data;
    } catch { /* kein Profil vorhanden */ }
  }
  return profiles;
