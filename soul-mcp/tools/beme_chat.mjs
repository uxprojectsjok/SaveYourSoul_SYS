...
export function register(server, token) {
  server.tool(
    ...
    ].join('\n'),
    {
      ...
      access_token: z.string().optional().describe('pol_access_token aus soul_pay() — für Gespräch mit fremder Soul'),
    },
    async ({ message, history, max_tokens, access_token }) => {
      const useToken = access_token || token;
      const isPaid   = !!access_token;
      const endpoint = isPaid ? '/api/soul/paid-beme' : '/api/beme';
      try {
        const data = await postJson(endpoint, useToken, {
          message,
          history:    history    ?? [],
