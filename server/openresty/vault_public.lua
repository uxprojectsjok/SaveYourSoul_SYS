...
if uri == "/api/vault/public/config" and method == "PUT" then
  ...
  if type(incoming.enabled) == "boolean" then current.enabled = incoming.enabled end
...
if uri == "/api/vault/public/sync" then
  ...
  if #decoded > 50 * 1024 * 1024 then
    ngx.status = 413; ngx.say('{"error":"File too large (max 50 MB)"}'); return
  end
...
if not config.enabled then
  ngx.status = 403; ngx.say('{"error":"Public vault not enabled"}'); return
end
