...
| pol_access_token   | `{48hexchars}`        | Paid AI agents — on-chain payment credential   |
...
| Tool                                | Description                                               |
| ----------------------------------- | --------------------------------------------------------- |
| `soul_read`                         | Read the full sys.md (owner) or AGENT block (paid agent) |
| `soul_write`                        | Update a `## Section` in sys.md or AGENT block            |
| `soul_maturity`                     | Compute soul maturity score (0–100)                       |
| `soul_skills`                       | Generate Claude skill files from soul data                |
| `soul_discover`                     | Search the public soul registry (no auth)                 |
| `soul_earnings`                     | Earnings ledger (soul owner only)                         |
| `profile_get` / `profile_save`      | Read and write structured profiles (face/voice/motion)    |
| `audio_list` / `audio_get`          | List and retrieve audio vault files                       |
| `image_list` / `image_get`          | List and retrieve image vault files (returned as base64)  |
| `video_list` / `video_get`          | List and retrieve video frames (ffmpeg extracted)         |
| `context_list` / `context_get`      | Context file access (.md/.txt only)                       |
| `calendar_read`                     | Read `## Calendar` section                                |
| `beme_chat`                         | Talk to the soul — AI responds as the owner               |
| `network_list` / `network_peer_get` | Soul Network peer connections                             |
...
| -------- | ------ | ----- | ------------------------------------------------- |
| `chat`   | 2 r/s  | 20    | `/api/chat`, `/api/soul-update`, vision endpoints |
