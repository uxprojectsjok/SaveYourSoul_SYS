...
The sys.md MAY contain a sandboxed section for external AI agent writes:
...
**Rules:**
- Only one AGENT block per file. If no block exists, `POST /api/soul/paid-write` appends one.
- Paid agents (pol_access_token) may only read and write within this block — the rest of sys.md is never exposed to them.
- `GET /api/soul/paid-read` returns only the content between the markers.
- `POST /api/soul/paid-write` strips any `<!-- AGENT:START -->` or `<!-- AGENT:END -->` sequences from incoming content before writing (injection protection). Agents cannot escape the block regardless of input.
- The block is preserved as-is by compliant parsers. Its content is freeform Markdown.
