...
## 7. Soul Network — Peer Vault Access

Connected souls (mutual connection via `soul_connections.json`) can read
each other's public vault content via:
...
This returns the connected souls' manifests and permitted content.
Vault files (audio, images) are only shared if explicitly uploaded
to the VPS in a connection-accessible state.
