...
## 10. Paid agent flow (Agent Marketplace)
...
# → { soul_id, name, mcp_endpoint, price_pol, wallet, free_tools, ... }

SOUL_ID="uuid-of-target-soul"

# 2. Pay on-chain (POL on Polygon) and receive access token
ACCESS_TOKEN=$(curl -s -X POST "$BASE/api/soul/pay" \
  -H "Content-Type: application/json" \
  -d "{\"tx_hash\":\"0x...\",\"soul_id\":\"$SOUL_ID\"}" \
  | jq -r '.access_token')
# Token is valid for the duration configured by the soul owner

# 3. Read the AGENT block (publicly visible portion of sys.md)
curl -s "$BASE/api/soul/paid-read" \
  -H "Authorization: Bearer $ACCESS_TOKEN"

# 4. Write to the AGENT block
curl -s -X POST "$BASE/api/soul/paid-write" \
  -H "Authorization: Bearer $ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"content":"Analysis complete. Recommendation: ...","mode":"replace"}'
# → { "ok": true, "section": "Agent-Kontext", "mode": "replace" }

# 5. Access vault files (subject to free_tools config)
curl -s "$BASE/api/vault/audio" \
...
## 11. Fetch encrypted bundle from Arweave
