...
OpenResty (VPS)
    ...
    ├── /api/chat          →  soul_auth.lua → Anthropic API (SSE)
    ├── /api/soul-update   →  soul_auth.lua → Anthropic API (JSON)
    ├── /api/soul-cert     →  soul_cert.lua (kein Auth)
    ├── /api/soul-sign-session  →  soul_sign_session.lua (kein Auth)
    ├── /api/validate      →  soul_auth.lua → 200 OK
    ├── /api/soul/v1/token →  soul_auth.lua → soul_token_jwt.lua
    ├── /api/vault/*       →  vault_auth.lua → vault_*.lua / api_serve.lua
    ├── /api/soul/*        →  vault_auth.lua → soul_*.lua
...
Alle API-Endpunkte werden von OpenResty Lua-Skripten bedient — **eine Ausnahme:**
`soul-mcp` läuft als separater Node.js-Prozess (PM2, Port 3098) für MCP + OAuth.

### Backend-Prozesse (PM2)

```bash
pm2 status                   # alle Prozesse anzeigen
pm2 restart soul-mcp         # nach Code-Änderungen neu starten
pm2 logs soul-mcp            # Logs einsehen
pm2 startup && pm2 save      # Autostart nach Reboot einrichten
```

| Prozess | Port | Proxied at |
|---|---|---|
| `soul-mcp` | `127.0.0.1:3098` | `/mcp`, `/oauth/`, `/.well-known/oauth-*`, `/health` |
...
└── lua/
    ...
    ├── vault_auth.lua                  # Token-Validierung (soul-cert | pol_access | service)
    ├── hmac_helper.lua                 # HMAC-SHA256 Helper, Cert-Derivation
    ├── soul_cert.lua                   # Cert-Generierung
    ├── soul_sign_session.lua           # Growth-Chain-Signierung
    ├── soul_token_jwt.lua              # JWT-Ausstellen
    ├── soul_rotate_cert.lua            # Cert-Rotation
    ├── soul_meta.lua                   # GET /api/soul/meta (öffentlich)
    ├── api_context.lua                 # GET/PUT /api/context
    ├── api_serve.lua                   # GET /api/soul, /api/vault/*
    ├── vault_sync.lua                  # POST /api/vault/sync (ClamAV + ffmpeg)
    ├── vault_unlock.lua                # POST /api/vault/unlock
    ├── vault_delete.lua                # DELETE /api/vault/{type}/{file}
    ├── vault_profile.lua               # GET/PUT/DELETE /api/vault/profile/{type}
    ├── vault_profile_analyze.lua       # POST /api/vault/profile/analyze
    ├── vault_public.lua                # /api/vault/public/* (Grants)
    ├── vault_services.lua              # /api/vault/services/* (Service-Token-Mgmt)
    ├── vault_connections_peer.lua      # /api/vault/peer-stream
    ├── soul_connections.lua            # /api/vault/connections/*
    ├── webhook.lua                     # POST /api/webhook
    ├── webhook_mnemonic.lua            # POST /api/webhook/mnemonic (BIP39-Auth)
    ├── external_vault.lua              # GET /api/vault/external/soul
    ├── fetch_bundle.lua                # POST /api/fetch-bundle (kein Auth)
    ├── vision_analyze.lua              # POST /api/vision-analyze
    ├── tts.lua                         # POST /api/tts (ElevenLabs)
    ├── wavespeed_submit.lua            # POST /api/wavespeed/submit
    ├── wavespeed_result.lua            # GET /api/wavespeed/result
    ├── beme.lua                        # POST /api/beme (beme_chat Backend)
    ├── soul_amortization.lua           # GET/PUT /api/soul/amortization
    ├── soul_pay.lua                    # POST /api/soul/pay (TX-Verifizierung)
    ├── soul_pol_validate.lua           # intern: Polygon RPC Validation
    ├── soul_verify_cache.lua           # intern: TX Replay-Schutz (48h)
    ├── soul_paid_read.lua              # GET /api/soul/paid-read
    ├── soul_paid_write.lua             # POST /api/soul/paid-write (AGENT-Block)
    ├── soul_paid_beme.lua              # POST /api/soul/paid-beme
    ├── soul_paid_context.lua           # GET /api/soul/paid-context
    ├── soul_paid_earnings.lua          # GET /api/soul/paid-earnings
    ├── soul_paid_profile.lua           # GET /api/soul/paid-profile/{type}
    ├── soul_register.lua               # POST /api/soul/register (IPFS)
    ├── soul_register_preview.lua       # GET /api/soul/register
    ├── soul_earnings.lua               # GET /api/soul/earnings
    └── soul_pinata_config.lua          # intern: Pinata IPFS API Helper
...
Vollständige Referenz: [docs/api/endpoints.md](api/endpoints.md)
...
| `/api/webhook`              | POST    | vault_auth           | webhook.lua            | Alle Ressourcen (eine Antwort)    |
| `/api/webhook/mnemonic`     | POST    | BIP39 im Body        | webhook_mnemonic.lua   | Ciphered-Zugriff via 12 Wörter    |
| `/mcp`                      | POST    | OAuth / pol_token    | soul-mcp (Node :3098)  | MCP Streamable HTTP               |
| `/oauth/*`                  | GET/POST| –                    | soul-mcp (Node :3098)  | OAuth 2.0 + PKCE                  |

### Agent Marketplace

| Endpunkt                    | Methode | Auth                 | Handler                | Zweck                             |
| --------------------------- | ------- | -------------------- | ---------------------- | --------------------------------- |
| `/api/soul/discover`        | GET     | –                    | soul_register.lua      | Öffentliche Soul-Suche            |
| `/api/soul/register`        | GET/POST | soul_cert           | soul_register.lua      | IPFS-Registrierung                |
| `/api/soul/amortization`    | GET/PUT | soul_cert            | soul_amortization.lua  | Marketplace-Konfiguration         |
| `/api/soul/pay`             | POST    | –                    | soul_pay.lua           | TX-Verifizierung → pol_access_token |
| `/api/soul/earnings`        | GET     | soul_cert            | soul_earnings.lua      | Zahlungshistorie                  |
| `/api/soul/paid-read`       | GET     | pol_access_token     | soul_paid_read.lua     | AGENT-Block lesen                 |
| `/api/soul/paid-write`      | POST    | pol_access_token     | soul_paid_write.lua    | AGENT-Block schreiben             |
