...
/**
 * Gibt den Agent-Bereich einer sys.md zurück (zwischen den Markern).
 * @param {string} markdown
 * @returns {string|null} Inhalt oder null wenn kein Bereich vorhanden
 */
export function getAgentSection(markdown) {
  if (!markdown) return null;
  const s = markdown.indexOf(AGENT_START);
  const e = markdown.indexOf(AGENT_END);
  if (s === -1 || e === -1 || e <= s) return null;
  return markdown.slice(s + AGENT_START.length, e).trim() || null;
}

/**
 * Setzt / ersetzt den Agent-Bereich in einer sys.md.
 * Wenn noch kein Bereich vorhanden, wird er ans Ende angehängt.
 * @param {string} markdown
 * @param {string} agentContent
 * @returns {string} aktualisiertes Markdown
 */
export function setAgentSection(markdown, agentContent) {
  const block = `\n${AGENT_START}\n${agentContent.trim()}\n${AGENT_END}\n`;
  const s = markdown.indexOf(AGENT_START);
  const e = markdown.indexOf(AGENT_END);
  if (s !== -1 && e !== -1 && e > s) {
    return markdown.slice(0, s) + block + markdown.slice(e + AGENT_END.length);
  }
  return markdown.trimEnd() + '\n' + block;
}
